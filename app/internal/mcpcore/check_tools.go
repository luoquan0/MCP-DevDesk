	...
	return []Tool{
		{
			...
			Description: "Auto-detect the project toolchain and run a trusted test, check, or format verification without guessing unknown commands.",
		...
		},
		{
			...
			Description: "Auto-detect a trusted project validation workflow from build.ps1, go.mod, package.json, Cargo.toml, .sln, or Python metadata, then run the recognized test/check/build phases in a terminal session.",
			InputSchema: map[string]any{"type": "object", "properties": map[string]any{
				"path":       map[string]any{"type": "string", "default": "."},
				"waitMillis": map[string]any{"type": "integer", "minimum": 0, "maximum": 30000, "default": 30000},
...
func (s *Server) executeCheckTool(name string, arguments map[string]any) (map[string]any, error) {
	...
	result, err := s.commands.start(execCommandArgs{Command: detected.Command, Args: detected.Args, CWD: display, TimeoutSeconds: 900, WaitMillis: wait})
	if err != nil {
		return nil, err
	}
...
func (s *Server) executeValidateProject(arguments map[string]any) (map[string]any, error) {
	...
	result, err := s.commands.start(execCommandArgs{Command: command, Args: commandArgs, CWD: display, TimeoutSeconds: 1800, WaitMillis: wait})
	if err != nil {
		return nil, err
	}
