...
func TestValidationDoesNotGuessUnknownCommands(t *testing.T) {
	dir := t.TempDir()
	if err := os.WriteFile(filepath.Join(dir, "package.json"), []byte(`{"scripts":{"deploy":"custom-deploy"}}`), 0o600); err != nil {
		t.Fatal(err)
	}
	if _, err := detectValidationPlan(dir); err == nil {
		t.Fatal("validation unexpectedly guessed a command for an unknown Node script")
	}
}
