...
func (s *Server) screenVisionDefaultInstructions() string {
	...
	switch policy.mode {
	case "desktop":
		return "When the user asks what a named, open, or background application window currently displays, treat it as a GUI-content question and use Screen Vision before process, port, service, or command metadata. Use screen_list_windows to locate the app and screen_capture_window to inspect its pixels. When the user asks what the current foreground window displays, use screen_capture_active_window. For background, minimized, or tray-hidden targets, call screen_capture_window directly: capture first tries state-neutral HWND rendering and Windows Graphics Capture, and only if those fail may resume a dormant target outside the visible virtual desktop with no activation/no Z-order change before restoring its prior placement and state. Do not ask the user to foreground the target before trying Screen Vision. Process/port metadata may supplement the visual result but does not answer what the GUI shows. screen_capture_desktop is for a desktop overview, not a substitute for capturing a named background app."
	case "window":
		return "When the user asks what the selected or locked application window displays, use screen_capture_window first. The locked target may be behind another app or minimized/tray-hidden. Capture first uses state-neutral PrintWindow/window-owned rendering and Windows Graphics Capture; only after those fail may a dormant target be resumed outside the visible virtual desktop with SW_SHOWNOACTIVATE plus no-activate/no-Z-order positioning, then returned to its original placement, minimized/hidden state, Z-order, and foreground focus. Do not ask the user to foreground the target before trying Screen Vision. Only request foregrounding after the capture tool itself fails. Process/port metadata may supplement the result but cannot replace GUI inspection."
	default:
