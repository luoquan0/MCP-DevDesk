...
func (s *Server) screenVisionDefaultInstructions() string {
	...
	switch policy.mode {
	case "desktop":
		return "When the user asks what a named, open, or background application window currently displays, treat it as a GUI-content question and use Screen Vision before process, port, service, or command metadata. Use screen_list_windows to locate the app and screen_capture_window to inspect its pixels. When the user asks what the current foreground window displays, use screen_capture_active_window. For background, minimized, or tray-hidden targets, call screen_capture_window directly: capture uses isolated state-neutral Windows Graphics Capture and PrintWindow. It never restores, clips, cloaks, moves or activates the target. A dormant renderer that supplies no usable pixels returns an explicit error, not a substitute desktop image. Do not ask the user to foreground the target before trying Screen Vision. Only ask the user to foreground it after an actual Screen Vision capture attempt fails. Process/port metadata may supplement the visual result but does not answer what the GUI shows. screen_capture_desktop is for a desktop overview, not a substitute for capturing a named background app."
	case "window":
		return "When the user asks what the selected or locked application window displays, use screen_capture_window first. The locked target may be behind another app or minimized/tray-hidden. Capture uses isolated state-neutral Windows Graphics Capture and PrintWindow, never changes target window state, and reports unavailable frames explicitly. Do not promise fresh frames from a suspended renderer. Do not ask the user to foreground the target before trying Screen Vision. Only ask the user to foreground it after an actual Screen Vision capture attempt fails. Only request foregrounding after the capture tool itself fails. Process/port metadata may supplement the result but cannot replace GUI inspection."
	default:
