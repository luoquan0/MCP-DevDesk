...
const (
	maxOutput          = 512 * 1024
	maxGitCommandBytes = 2 * 1024 * 1024
...
func GetDiff(path string) (Diff, error) {
	...
	out, truncated, err := gitBytesLimited(path, maxOutput+1, "diff", "--no-ext-diff", "--no-color")
	if err != nil {
		return Diff{}, err
	}
...
func gitIsAncestor(root, target, head string) error {
	if _, err := git(root, "merge-base", "--is-ancestor", target, head); err != nil {
...
func uniqueBackupBranch(root, previous string) string {
	...
	for suffix := 0; ; suffix++ {
		...
		if _, err := git(root, "show-ref", "--verify", "--quiet", "refs/heads/"+candidate); err != nil {
			return candidate
		}
	}
}
...
func gitBytes(path string, args ...string) ([]byte, error) {
	out, _, err := gitBytesLimited(path, maxGitCommandBytes, args...)
	return out, err
}

func gitBytesLimited(path string, limit int, args ...string) ([]byte, bool, error) {
	...
	cmd.Cancel = func() error { return terminateGitProcess(cmd) }
	cmd.WaitDelay = 5 * time.Second
	stdout := &limitedGitBuffer{limit: limit}
	stderr := &limitedGitBuffer{limit: 64 * 1024}
	cmd.Stdout, cmd.Stderr = stdout, stderr
	if err := cmd.Run(); err != nil {
		if ctx.Err() != nil {
			return nil, false, fmt.Errorf("git %s timed out", strings.Join(args, " "))
		}
...
func (b *limitedGitBuffer) Write(data []byte) (int, error) {
	original := len(data)
