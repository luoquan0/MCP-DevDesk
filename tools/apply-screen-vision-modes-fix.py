...
# Model: persist Screen Vision policy and expose visible-window metadata through GET /api/config.
replace_once(
...
# Config: backward-compatible defaults, normalization, validation and partial updates.
replace_once(
    "app/internal/config/store.go",
...
\t// Dangerous and trusted modes always support networking.''',
    '''\tcfg.ProxyAddress = strings.TrimSpace(cfg.ProxyAddress)
\tcfg.ProxyUsername = strings.TrimSpace(cfg.ProxyUsername)
...
# Application GET /api/config: include current visible-window metadata without persisting it.
replace_once(
...
# MCP tool execution: enforce selected mode and lock specified-window capture to the user-selected target.
replace_once(
...
# Go MCP Core reads the persisted Screen Vision policy from the same local config file already supplied for logging.
replace_once(
...
# Updater: allow only the special prerelease -> latest stable transition; stable builds remain upgrade-only.
replace_once(
    "app/internal/updater/manager.go",
...
# Rich Screen Vision UI: immediate persistence fixes the old toggle-reset bug.
security_component = r'''<script setup lang="ts">
...
'''
Path("frontend/src/components/settings/SecuritySettingsSection.vue").write_text(security_component, encoding="utf-8")

# Documentation stays aligned with the new least-privilege capture modes and stable-return behavior.
replace_once(
...
5. 下载对应 `.sha256` 并解析预期校验值。''',
    '''4. 使用语义版本比较判断是否存在更新。稳定版正常情况下只允许向更高版本升级；唯一的受控例外是“当前程序为 prerelease/beta 且用户主动切换到稳定通道”，此时允许安装 GitHub 最新稳定版，即使它的版本号低于当前测试版，从而可以安全退出测试通道。
5. 下载对应 `.sha256` 并解析预期校验值。''',
)

# Tests for mcp-core reading persisted capture policy without leaking titles through process arguments.
Path("app/cmd/mcp-core/screen_vision_config_test.go").write_text(r'''package main
