...
jobs:
  windows-amd64:
    ...
    steps:
      ...
      - name: Guard public repository secrets
        shell: pwsh
        run: .\\tools\\check-public-release.ps1

      - name: Build and test
        shell: pwsh
        env:
          MCP_DEVDESK_GITHUB_REPOSITORY: ${{ github.repository }}
        run: .\\build.ps1 -Arch amd64 -RunTests
'''
Path(".github/workflows/ci.yml").write_text(original_ci, encoding="utf-8")

for temp in [
    Path(".github/workflows/apply-updater-timeout-fix.yml"),
    Path("tools/apply_updater_timeout_fix.py"),
]:
    if temp.exists():
        temp.unlink()
