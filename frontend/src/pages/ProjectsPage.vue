...
const completionPromptTemplate = `# 项目 Agent 指令

## 执行方式
- 收到任务后先完整理解需求，并持续执行到当前任务完成，除非遇到必须由用户提供信息、授权或外部条件才能继续的真实阻塞。
- 当前环境允许继续时不要把同一任务拆成“下一阶段再做”，也不要为了汇报进度而停止执行。
- 用户明确要求只分析、只评估或不要修改时，以用户当前指令为准。

## 验证要求
- 涉及代码修改时，完成必要的读取、修改、格式化、测试、构建或验证后再给最终回复。
- 优先使用项目已经存在的测试、Lint、构建和检查命令；失败时说明真实失败原因。

## 完成标准
- 最终回复只总结已经完成的事项、验证结果和真实阻塞项。
- 没有阻塞时，不把本轮能够完成的工作留到下一轮。`;
  ...
  <div class="page-stack projects-page">
    ...
    <section class="project-list">
      <AppCard v-for="project in app.projects" :key="project.id" class="project-list-card">
        ...
        <div class="current-project-copy">
          <div class="current-project-title"><h3>{{ project.name }}</h3><StatusPill v-if="isActive(project.path)" tone="success">当前活动</StatusPill><StatusPill v-if="project.prompt" tone="info">AGENTS.md</StatusPill></div>
        ...
        <div class="project-row-actions">
          ...
          <template v-else>
            ...
            <AppButton tone="secondary" icon="settings" @click="startPromptEdit(project)">AGENTS.md</AppButton>
        ...
        <div v-if="promptEditingId === project.id" class="project-prompt-editor">
          <div class="card-heading">
            <div><span class="eyebrow">Project AI instructions</span><h3>{{ project.name }} · AGENTS.md</h3><p>直接编辑项目根目录的标准 Agent 指令文件；项目复制或换工具后仍可复用。</p></div>
          </div>
          <label class="field project-prompt-field">
            <span>AGENTS.md 内容</span>
            <textarea v-model="promptDraft" rows="10" spellcheck="false" placeholder="# 项目说明&#10;&#10;## 开发规则&#10;- 明确代码风格、禁止事项和业务约束&#10;&#10;## 验证命令&#10;- 列出必须运行的测试/构建命令&#10;&#10;## 完成标准&#10;- 说明任务完成前必须满足的条件" />
            <small>{{ projectPromptBytes }} / {{ app.projectPromptSettings?.maxPromptBytes || 32768 }} bytes · 保存位置：{{ project.path }}\AGENTS.md</small>
          </label>
          <div class="form-footer">
            <small>保存会直接写入项目根目录的 AGENTS.md；留空保存会删除该文件。若“设置 → 全局提示词”已开启且内容非空，运行时会额外叠加全局规则，但不会把全局规则写进项目目录。</small>
            <div class="form-footer-actions">
              ...
              <AppButton tone="primary" :loading="app.actionPending === `prompt-${project.id}`" @click="saveProjectPrompt(project)">保存 AGENTS.md</AppButton>
            </div>
          </div>
        </div>
      </AppCard>
