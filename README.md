...
`0.11.0` 提供可直接使用的独立 Go MCP 核心，并兼容 OAuth 弹窗的 opaque/null Origin、授权回调 CSP、项目根目录 `AGENTS.md` 热更新、可开关全局提示词、聊天连接缓存兜底，以及带可选密码认证且与桌面软件 1:1 共用界面的局域网网页端。新建实例建议使用 Go 核心；Python 核心继续作为兼容回退。已经添加到 ChatGPT 的实例不建议在同一域名下频繁切换核心，因为切换后可能需要重新授权：
