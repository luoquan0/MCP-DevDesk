...
- 全局或单项目提示词保存后立即同步到对应 Go MCP 实例；运行核心检测指令变化后会废弃旧 MCP 会话，促使客户端自动重新握手并读取最新提示词
- Go 核心新增 `get_instructions` 工具，可直接读取当前实际生效的工作区规则、全局提示词和项目提示词，便于验证注入状态
...
`0.8.8` 提供可直接使用的独立 Go MCP 核心，并兼容 OAuth 弹窗的 opaque/null Origin、授权回调 CSP，以及项目提示词的运行时同步、自动重新握手和聊天连接缓存兜底。新建实例建议使用 Go 核心；Python 核心继续作为兼容回退。已经添加到 ChatGPT 的实例不建议在同一域名下频繁切换核心，因为切换后可能需要重新授权：
