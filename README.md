...
- 网页控制台支持可选局域网访问，独立绑定端口且不开放内部 17860 管理 API；远程控制面只暴露项目、目录、提示词和服务操作
- 网页控制密码独立保存在 DPAPI 加密的 `secrets.json` 中；可选启用登录认证，登录会话使用 HttpOnly + SameSite Cookie 且程序重启后自动失效
- 手机网页拆分为登录、项目、提示词和服务页面，底部导航切换；独立滚动容器避免桌面 UI 的 `overflow:hidden` 影响移动端滚动
...
`0.10.0` 提供可直接使用的独立 Go MCP 核心，并兼容 OAuth 弹窗的 opaque/null Origin、授权回调 CSP、项目根目录 `AGENTS.md` 热更新、可开关全局提示词、聊天连接缓存兜底，以及带可选密码认证的手机/局域网网页控制台。新建实例建议使用 Go 核心；Python 核心继续作为兼容回退。已经添加到 ChatGPT 的实例不建议在同一域名下频繁切换核心，因为切换后可能需要重新授权：
