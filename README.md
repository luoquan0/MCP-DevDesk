...
`0.8.5` 提供可直接使用的独立 Go MCP 核心，并兼容部分 OAuth 弹窗在授权表单提交时使用的 opaque/null Origin。新建实例建议使用 Go 核心；Python 核心继续作为兼容回退。已经添加到 ChatGPT 的实例不建议在同一域名下频繁切换核心，因为切换后可能需要重新授权：
