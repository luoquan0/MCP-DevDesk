...
- 设置页新增“网页控制”开关、独立端口和一键打开入口；网页控制与内部 17860 管理端口、MCP 端口相互独立，并会阻止端口冲突
- 新增轻量 `/#/control` 浏览器控制台，第一版集中提供项目目录切换、项目根目录 `AGENTS.md` 编辑、全局提示词开关/编辑，以及 MCP 启动、停止、重启
- 网页控制默认关闭并仅监听 `127.0.0.1`；本版本不会直接暴露到局域网或公网，后续开放远程访问时需配套独立认证
...
`0.9.0` 提供可直接使用的独立 Go MCP 核心，并兼容 OAuth 弹窗的 opaque/null Origin、授权回调 CSP、项目根目录 `AGENTS.md` 热更新、可开关全局提示词、聊天连接缓存兜底，以及可选的本机网页控制台。新建实例建议使用 Go 核心；Python 核心继续作为兼容回退。已经添加到 ChatGPT 的实例不建议在同一域名下频繁切换核心，因为切换后可能需要重新授权：
