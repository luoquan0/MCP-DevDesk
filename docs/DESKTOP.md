...
beta.4 的跨进程空 Region/离屏恢复方案已从修复分支移除。Screen Vision 不再为了截图更改目标软件的原生窗口属性或显示状态，也不使用管理器单实例唤醒流程执行捕获。捕获 worker 必须在单实例和配置处理之前分流，避免截图意外打开另一管理器窗口。最小化/隐藏软件不提供有效像素时明确失败，不强行恢复；实际兼容性和旧窗口恢复注意事项见 `docs/SCREEN_VISION.md`。
