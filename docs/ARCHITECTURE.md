...
屏幕截图请求在原有权限检查后，由同一可执行文件的本地管道 worker 执行。该入口先于单实例、管理器配置及 HTTP/OAuth 初始化，不产生第二个管理器。worker 有父进程超时和独立退出期限，使用环境白名单，只在内存中处理图像。窗口捕获禁止修改目标 Region、Cloak、显示状态、位置或焦点；详情见 `docs/SCREEN_VISION.md`。
