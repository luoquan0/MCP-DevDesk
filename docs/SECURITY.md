...
捕获 helper 是用户权限下的本地子进程，不是新增 MCP 操作或网络服务。父进程先检查现有模式与授权，再通过有界管道传递 HWND/PID；helper 不加载管理配置或 OAuth 服务，环境仅保留必要 Windows 路径。原生操作超时只回收自己的 helper，绝不结束目标进程。截图代码不再恢复/移动/裁剪/激活第三方窗口，不通过丢失的旧状态快照尝试全桌面修复。
