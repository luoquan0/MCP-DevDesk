...
- 网页控制使用独立端口，不复用内部 `17860` 管理后台，也不会把诊断、OAuth 密钥或完整实例管理 API 暴露到局域网。
- 未开启“允许局域网访问”时只监听 `127.0.0.1`；开启后监听 IPv4 网卡，但请求源仍必须是 loopback 或 RFC1918 私有地址。
- 网页控制仅开放项目列表/切换/目录浏览、项目 `AGENTS.md`、全局提示词和主 MCP 服务启停/重启。
...
- 目录浏览接口只枚举目录，不返回文件内容；其能力范围等同于当前 Windows 用户可访问的目录，因此局域网模式建议始终同时启用密码认证。
