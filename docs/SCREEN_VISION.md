...
- `指定窗口`：用户先在 DevDesk 的 Windows 可见窗口列表中手动选择目标。运行时以窗口 ID + 进程 ID 锁定；目标关闭或身份变化后不会自动改抓其他窗口，必须重新选择。最小化窗口不会出现在可选列表中；应先恢复目标窗口再刷新。后台窗口如果无法自行渲染截图，DevDesk 会返回错误，而不会退回到同一屏幕坐标去误抓覆盖在上面的其他窗口。
...
| 工具 | 用途 |
| --- | --- |
| `screen_list_windows` | 列出当前可见、非最小化的顶层应用窗口，可按标题或进程名筛选。 |
...
窗口枚举会通过 `IsIconic` 排除已最小化窗口。若已锁定窗口在选择之后被最小化，截图调用会直接报错并要求恢复窗口、刷新后重新选择，不会把最小化后的标题条/代理窗口当成应用主画面。
...
4. 测试“指定窗口”时，先把目标应用恢复为正常窗口，再在 DevDesk 刷新列表并选择目标；让客户端调用 `screen_capture_window`。即使另一个应用成为前台，也只能返回锁定目标自身画面或明确报错，不应返回前台应用画面。
5. 若有多个 MCP 实例，刻意让 ChatGPT 连接一个附加实例，再重复指定窗口测试，确认其权限与主设置完全一致。
6. 测试“整个桌面”时，客户端应能同时看到 `screen_list_windows`、`screen_capture_window` 和 `screen_capture_desktop`；先列出窗口，再逐个截图验证。
7. 测试完成后关闭 Screen Vision；视觉工具应从所有 Go MCP 实例的工具列表消失。

建议重点反馈：窗口是否能被正确列出、指定窗口是否始终对应正确程序、多实例连接下权限是否一致、被遮挡窗口是否能单独读取、截图是否黑屏、DPI/多显示器下截图范围是否正确、截图延迟、开启/关闭后的空闲资源占用，以及所使用的 Windows 版本和目标应用。